...
pub(crate) async fn function_handler(event: LambdaEvent<SqsEvent>) -> Result<(), Error> {
    ...
    let mut event_map: HashMap<tuple<String, String>, Vec<&SqsMessage>> = HashMap::new();
...
fn get_namespace_and_table(message: &SqsMessage) -> tuple<String, String> {
    let body: String = message.body.to_owned().unwrap();
    let parsed_body: Value = serde_json::from_str(&body[..]).unwrap();
    let detail_type = parsed_body["detail-type"].as_str().unwrap();
    let parts: Vec<&str> = String::from(detail_type).split(".").collect();
    return (String::from("dfs_") + parts[0], parts[1]);
}
